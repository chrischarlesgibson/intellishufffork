...
export class RegisterComponent extends BasePage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.institutions = await this.authSvc.getAllInstitutions();
  }
  ...
  async onFormSubmitted(data: IRegister) {
    ...
    const params = {
      ...
      institution: this.fg['institution'].value
    };

    try {
      const resp: IResponse<any> = await this.authSvc.regsiter(params as any);
      if (resp.status) {
