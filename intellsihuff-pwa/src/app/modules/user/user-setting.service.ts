...
export class UserSettingService extends AppSettingService {
    ...
    putAccessToken(token:any) {
        return this.dbService.putLocal(this.schemaSvc.tables.setting, {
            ...
            AppSettingService.settingCache.set(UserConstant.KEY_ACCESS_TOKEN, token);
        });
    }
    
    removeAccessToken() {
        return this.dbService.remove(this.schemaSvc.tables.setting, UserConstant.KEY_ACCESS_TOKEN)
        .then(() => {
    ...
    getAccessToken() {
        return this.get<string>(UserConstant.KEY_ACCESS_TOKEN)
            .then(token => {
                return token;
            });
    }
